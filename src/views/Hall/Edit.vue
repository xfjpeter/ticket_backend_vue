...
export default {
  ...
  created () {
    this.$http.get({ url: `/hall/${this.$route.query.id}` })
      .then(res => {
        if (!res.err_code) {
          this.ruleForm = res.data
          this.sites = res.data.map
          this.rows = res.data.map.length || 0
          this.cols = res.data.map ? res.data.map[0].length : 0
        }
      })
  },
  methods: {
    submit (ruleForm) {
      this.$refs[ruleForm].validate(valid => {
        if (valid) {
          ...
          this.$http.post({ url: `/hall/${this.ruleForm.id}/save`, data: this.ruleForm })
            .then(res => {
              if (!res.err_code) {
                this.$notify.success({
                  title: '提示',
