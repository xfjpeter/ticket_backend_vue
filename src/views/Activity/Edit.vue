  ...
  <div class="activity-add container-inner">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
      ...
      <el-form-item prop="time" label="活动时间">
        <el-date-picker v-model="time" start-placeholder="开始日期" end-placeholder="结束日期" type="datetimerange" value-format="yyyy-MM-dd HH:mm"></el-date-picker>
      </el-form-item>
     ...
     <el-form-item prop="hall_id" label="厅(座位)">
       <el-select v-model="ruleForm.hall_id" disabled>
...
export default {
  ...
  created () {
    this.getHall()
    this.getForm()
    this.$http.get({ url: `/activity/${this.$route.query.no}` })
      .then(res => {
        if (!res.err_code) {
    ...
    submit (ruleForm) {
      this.$refs[ruleForm].validate(valid => {
        if (valid) {
          ...
          this.$http.post({ url: `/activity/${this.ruleForm.no}/save`, data: this.ruleForm })
            .then(res => {
              if (!res.err_code) {
                this.$notify.success({
                  title: '提示',
